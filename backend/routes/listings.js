...
router.get("/retrieve", async (req, res) => {
  try {
    ...
    const filter = storeName ? { store: storeName } : { status: "published" };
    ...
    try {
      ...
      const total = await Listing.countDocuments(filter).catch(
        () => mappedFallback.length
      );

      return res.json({
        results: mappedFallback,
        total,
        page: pageNum,
        limit: lim,
      });
