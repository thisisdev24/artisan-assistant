    ...
    try {
        ...
        try {
            ...
            if (models.DailyStats) {
                const recentStats = await models.DailyStats.find({ date: { $gte: startDate } })
                    .sort({ date: -1 })
                    .limit(days);

                if (recentStats.length > 0) {
                    // Aggregate from daily stats
                    analyticsData.totalRevenue = recentStats.reduce((sum, s) =>
                        sum + (s.revenue?.gross_revenue || 0), 0);
                    analyticsData.conversionRate = recentStats.reduce((sum, s) =>
                        sum + (s.engagement?.conversion_rate_pct || 0), 0) / recentStats.length;
                    analyticsData.bounceRate = recentStats.reduce((sum, s) =>
                        sum + (s.engagement?.bounce_rate_pct || 0), 0) / recentStats.length;
                    analyticsData.avgSessionDuration = recentStats.reduce((sum, s) =>
                        sum + (s.engagement?.avg_session_duration_sec || 0), 0) / recentStats.length;
                    analyticsData.uniqueVisitors = recentStats.reduce((sum, s) =>
                        sum + (s.users?.active_users || 0), 0);
                }
            }
            ...
            if (models.SalesSummary) {
                ...
                if (recentSales.length > 0) {
                    analyticsData.totalOrders = recentSales.reduce((sum, s) =>
                        sum + (s.totals?.orders || 0), 0);
                    analyticsData.totalRevenue = recentSales.reduce((sum, s) =>
                        sum + (s.totals?.revenue || 0), 0) || analyticsData.totalRevenue;
                    analyticsData.avgOrderValue = analyticsData.totalOrders > 0
            ...
            if (models.TrafficOverview) {
                ...
                if (recentTraffic.length > 0) {
                    analyticsData.pageViews = recentTraffic.reduce((sum, t) =>
                        sum + (t.total_sessions || 0), 0);
                    analyticsData.uniqueVisitors = recentTraffic.reduce((sum, t) =>
                        sum + (t.total_users || 0), 0) || analyticsData.uniqueVisitors;
                }
            }
        } catch (analyticsErr) {
            console.log("Analytics DB not available, using defaults:", analyticsErr.message);
        }
...
router.get("/analytics/top-products", async (req, res) => {
    try {
        ...
        try {
            ...
            if (models.SalesSummary) {
                const latest = await models.SalesSummary.findOne().sort({ end_date: -1 });
...
router.get("/analytics/devices", async (req, res) => {
    try {
        ...
        try {
            ...
            if (models.TrafficOverview) {
                const latest = await models.TrafficOverview.findOne().sort({ date: -1 });
                if (latest && latest.devices && latest.devices.length > 0) {
                    const totalSessions = latest.devices.reduce((sum, d) => sum + (d.sessions || 0), 0);
                    if (totalSessions > 0) {
                        deviceStats = {
                            desktop: 0,
                            mobile: 0,
