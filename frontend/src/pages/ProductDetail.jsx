...
const ProductDetail = () => {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        const initialImage =
          ...
          (data.images &&
            data.images[0] &&
            (data.images[0].large || data.images[0].thumb));
  ...
  const allImages = () => {
    ...
    if (product.images && Array.isArray(product.images)) {
      product.images.forEach((img) => {
        ...
        else if (img.large && !images.includes(img.large))
  ...
  const talkToSeller = () => {
    ...
    if (seller && typeof seller === "object" && seller.email) {
      window.location.href = `mailto:${
        seller.email
      }?subject=Inquiry about ${encodeURIComponent(product.title)}`;
      return;
    }
    if (seller && typeof seller === "string") {
      navigate(
        `/contact?seller=${encodeURIComponent(seller)}&productId=${product._id}`
      );
      return;
    }
    navigate("/contact");
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="w-full max-w-xl md:max-w-7xl mx-auto py-24">
        {/* Header */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-6">
            <div className="relative bg-transparent rounded-2xl hover:shadow-2xl overflow-hidden p-4 shadow-xl">
              <img
                src={mainImage}
                alt={product.title}
                className="w-[400px] aspect-square object-contain mx-auto rounded-xl bg-white transition-transform duration-400 ease-out hover:scale-105"
              />
            </div>
            ...
            <div className="bg-transparent rounded-2xl shadow-lg p-4 lg:p-6">
              ...
              <div className="mt-4 flex items-center gap-4">
                ...
                <div className="text-sm text-gray-600">
                  {avg ? `${avg.toFixed(1)} • ` : ""}
                  {product.rating_number ?? 0} ratings
                </div>
                <div className="ml-auto text-sm text-gray-500">
                  SKU:{" "}
                  <span className="font-mono">
                    {product.sku || product._id.slice(0, 8)}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4 mb-8 flex items-center gap-4">
                <div className="text-3xl lg:text-4xl font-extrabold text-red-700">
              ...
              <div className="flex flex-col items-start gap-4 text-lg font-semibold">
                Product Features
                {product.features &&
              ...
              {/* Description */}
              <div className="mt-8 flex flex-col items-start gap-4 text-lg font-semibold">
                Product Description
              ...
              <div className="mt-8 flex items-center gap-4 bg-transparent p-4 rounded-lg">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-pink-500 flex items-center justify-center text-white font-semibold">
                  {product.seller && typeof product.seller === "string"
                    ? (product.seller[0] || "S").toUpperCase()
                    : "S"}
                </div>
                <div>
                  ...
                  <div className="font-semibold text-gray-900">
                    {product.seller && typeof product.seller === "object"
                      ...
                      : product.seller || "Anonymous Seller"}
                  </div>
                  <div className="text-sm text-gray-500">
                    {product.seller &&
                ...
                <div className="ml-auto flex gap-2">
                  ...
                  <button
                    onClick={() => alert("Seller follow placeholder")}
              ...
              <div className="mt-6 border-t pt-6">
                <div className="flex items-center gap-3">
                  <div className="flex items-center rounded-md border divide-x overflow-hidden">
                    ...
                    <div className="px-4 py-2 bg-white text-sm font-medium">
                      {qty}
                    </div>
                    <button
                      onClick={() =>
                  ...
                  <button
                    ...
                    className={`px-4 py-3 rounded-lg ${
                      product.stock === 0 ? "bg-gray-200" : "bg-white border"
                    }`}
                  >
                    Buy Now
                  </button>
        ...
        <div className="fixed left-0 right-0 bottom-0 sm:hidden bg-white/95 border-t py-3 px-4 flex items-center gap-3 justify-between">
          <div>
            ...
            <div className="font-semibold text-lg">
              ₹{currency(Math.floor(product.price * 80) * qty)}
            </div>
          </div>
          <div className="flex gap-2 w-1/2">
            <button
              onClick={handleAddToCart}
              disabled={product.stock === 0}
              className="flex-1 px-3 py-3 rounded-lg bg-indigo-600 text-white"
            >
              Add
            </button>
            <button
              onClick={handleBuyNow}
              disabled={product.stock === 0}
              className="flex-1 px-3 py-3 rounded-lg bg-white border"
            >
              Buy
            </button>
          </div>
  ...
  const fetchMyReview = async () => {
    try {
      const response = await apiClient.get(
        `/api/reviews/listing/${productId}/my-review`
      );
      if (response.data) {
